    ...
    class ClippedPolygon : Polygon
    {
        ...
        public override List<ColorPoint> GetPixels(params object[] param)
        {
            ...
            for (int i = 0; i <= points.Count - 2; i++)
                pixels.AddRange(new SymmetricMidPointLine(ofLine, thickness + 3, points[i], points[i + 1], clipper).GetPixels());
        ...
        public override List<ColorPoint> GetPixelsAA(Bitmap bmp)
        {
            ...
            for (int i = 0; i <= points.Count - 2; i++)
                pixels.AddRange(new SymmetricMidPointLine(shapeColor, thickness + 3, points[i], points[i + 1], clipper).GetPixelsAA(bmp));
